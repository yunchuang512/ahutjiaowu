...
public class ReceiverActivity extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		Log.d(TAG, "[ReceiverActivity] onReceive - " + intent.getAction()
				+ ", extras: " + printBundle(bundle));

		if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
			String regId = bundle
					.getString(JPushInterface.EXTRA_REGISTRATION_ID);
			Log.d(TAG, "[ReceiverActivity] 接收Registration Id : " + regId);
			// send the Registration Id to your server...

		} else if (JPushInterface.ACTION_MESSAGE_RECEIVED.equals(intent
				.getAction())) {
			Log.d(TAG,
					"[ReceiverActivity] 接收到推送下来的自定义消息: "
							+ bundle.getString(JPushInterface.EXTRA_MESSAGE));
			// processCustomMessage(context, bundle);

		} else if (JPushInterface.ACTION_NOTIFICATION_RECEIVED.equals(intent
				.getAction())) {
			Log.d(TAG, "[ReceiverActivity] 接收到推送下来的通知");
			int notifactionId = bundle
					.getInt(JPushInterface.EXTRA_NOTIFICATION_ID);
			Log.d(TAG, "[ReceiverActivity] 接收到推送下来的通知的ID: " + notifactionId);

		} else if (JPushInterface.ACTION_NOTIFICATION_OPENED.equals(intent
				.getAction())) {
			Log.d(TAG, "[ReceiverActivity] 用户点击打开了通知");
			...
			Intent i = new Intent(context, ReceiverDetailActivity.class);
			i.putExtras(bundle);
			...
			System.out.println("open activity");
				...
				.getAction())) {
			Log.d(TAG,
					"[ReceiverActivity] 用户收到到RICH PUSH CALLBACK: "
			...
			Log.e(TAG, "[ReceiverActivity]" + intent.getAction()
					+ " connected state change to " + connected);
		} else {
			Log.d(TAG,
					"[ReceiverActivity] Unhandled intent - "
							+ intent.getAction());
		}
	}
